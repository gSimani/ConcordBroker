  ...
  const loadDashboardData = async () => {
    ...
    try {
      const [statsRes, cityRes, typeRes, salesRes] = await Promise.all([
        fetch('http://localhost:8000/api/fast/stats'),
        fetch('http://localhost:8000/api/fast/cities'),
        fetch('http://localhost:8000/api/properties/stats/by-type'),
        fetch('http://localhost:8000/api/properties/high-value?min_value=500000&limit=10')
      ]);
      ...
      if (statsRes.ok) {
        ...
        if (statsData.success && statsData.data) {
          setStats({
            totalProperties: statsData.data.total_properties,
            totalValue: statsData.data.total_value,
            averageValue: statsData.data.avg_value,
            recentSales: 0,
            highValueCount: Math.floor(statsData.data.total_properties * 0.15),
            watchedCount: 0
          });
        }
      }
      if (cityRes.ok) {
        ...
        if (cityData.success && cityData.data) {
          setCityStats(cityData.data.map((city: any) => ({
            city: city.city,
            propertyCount: city.count,
            averageValue: city.avg_value,
            totalValue: city.count * city.avg_value,
            percentChange: 0
          })));
        }
      }
      ...
      if (salesRes.ok) {
        ...
        if (salesData.success && salesData.data) {
          setRecentSales(salesData.data.map((sale: any, index: number) => ({
            id: index + 1,
            address: sale.address || 'Unknown',
            city: sale.city || 'Unknown',
            saleDate: '2024-01-01',
            salePrice: sale.market_value || 0,
