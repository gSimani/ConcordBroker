...
async function fetchGrowthProjections(county: string, city: string) {
  console.log(`[fetchGrowthProjections] Fetching real growth data for ${city}, ${county}`);

  try {
    // Get historical sales trends to calculate real growth projections
    const { data: salesTrends, error } = await supabase
      .from('florida_parcels')
      .select('just_value, year')
      .eq('county', county)
      .eq('phy_city', city)
      .not('just_value', 'is', null)
      .gte('year', 2020)
    ...
    if (error) {
      console.log(`[fetchGrowthProjections] Error fetching sales trends: ${error.message}`);
      return [];
    }

    if (!salesTrends || salesTrends.length === 0) {
      console.log(`[fetchGrowthProjections] No sales trends data found for ${city}, ${county}`);
      return [];
    }

    // Calculate year-over-year growth rates from real data
    const yearlyGrowth = new Map<number, number>();
    const salesByYear = salesTrends.reduce((acc, sale) => {
      const year = parseInt(sale.year);
      const value = parseFloat(sale.just_value);
    ...
    const yearlyAverages: Array<{year: number, avgValue: number}> = [];
    Object.entries(salesByYear).forEach(([year, values]) => {
      const avgValue = values.reduce((sum, val) => sum + val, 0) / values.length;
      yearlyAverages.push({ year: parseInt(year), avgValue });
    });
    ...
    for (let i = 1; i < yearlyAverages.length; i++) {
      ...
      const previousYear = yearlyAverages[i - 1];
      const growthRate = ((currentYear.avgValue - previousYear.avgValue) / previousYear.avgValue) * 100;
    ...
    const avgGrowthRate = growthRates.length > 0
      ...
      : 2.5; // Conservative default if no data

    // Project future growth with gradual decline (realistic market assumption)
    const currentYear = new Date().getFullYear();
    const projections = [];
    for (let i = 1; i <= 5; i++) {
      const year = currentYear + i;
      // Gradual decline in growth rate over time (common economic pattern)
      const declineFactor = Math.max(0.8, 1 - (i * 0.1));
      const projectedGrowth = Math.max(1.0, avgGrowthRate * declineFactor);

      projections.push({
        year,
        projected_growth: Math.round(projectedGrowth * 10) / 10, // Round to 1 decimal
        data_source: 'historical_analysis',
        confidence: growthRates.length >= 3 ? 'high' : 'medium'
      });
    }

    console.log(`[fetchGrowthProjections] Real projections calculated for ${city}, ${county}:`, {
      historicalYears: yearlyAverages.length,
    ...
    console.error(`[fetchGrowthProjections] Error calculating growth projections:`, error);
    return [];
  }
}
