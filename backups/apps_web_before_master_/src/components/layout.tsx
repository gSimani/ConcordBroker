...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { generateId } = useElementId('layout')
  ...
  if (useModernLayout) {
    return (
      <div
        id={generateId('main', 'layout-container', 1)}
        data-testid="layout-modern-container"
        className="min-h-screen bg-gradient-to-br from-background via-background to-gray-50/30"
      >
        <Sidebar
        ...
        >
          <div
            id={generateId('main', 'content-wrapper', 1)}
            data-testid="layout-content-wrapper"
            className="relative"
          >
            {children}
          </div>
  ...
  return (
    <div
      id={generateId('main', 'legacy-container', 1)}
      data-testid="layout-legacy-container"
    ...
    >
      ...
      >
        <div
          id={generateId('header', 'container', 1)}
          data-testid="header-container"
          className="container mx-auto px-6 py-4"
        >
          <div
            id={generateId('header', 'content', 1)}
            data-testid="header-content"
            className="flex items-center justify-between"
          >
            <div
              id={generateId('header', 'brand-section', 1)}
              data-testid="header-brand"
              className="flex items-center space-x-4"
            >
              <div
                id={generateId('header', 'logo', 1)}
              ...
              <div
                id={generateId('header', 'brand-text', 1)}
      ...
      <footer className="border-t border-border/30 mt-12">
        <div
          id={generateId('footer', 'container', 1)}
          data-testid="footer-container"
          className="container mx-auto px-6 py-8"
        >
          <div
            id={generateId('footer', 'content', 1)}
            data-testid="footer-content"
            className="text-center"
          >
            <div
              id={generateId('footer', 'brand-section', 1)}
              data-testid="footer-brand"
              className="flex items-center justify-center mb-4"
            >
              <div
                id={generateId('footer', 'logo', 1)}
                data-testid="footer-logo"
                className="flex items-center justify-center w-6 h-6 rounded-lg bg-gradient-primary shadow-sm mr-3"
              >
