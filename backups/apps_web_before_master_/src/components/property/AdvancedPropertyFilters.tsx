  ...
  useEffect(() => {
    const fetchCounties = async () => {
      try {
        ...
        if (response.data) {
          setAvailableCounties(Array.isArray(response.data) ? response.data : []);
        }
      } catch (error) {
        console.error('Error fetching counties:', error);
        // Fallback to known counties if API fails
        setAvailableCounties(['HILLSBOROUGH', 'ORANGE']);
      } finally {
