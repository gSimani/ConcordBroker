...
export function ElegantPropertyTabs({
  ...
  const { generateId, generateTestId } = useElementId('property-tabs');
  ...
  return (
    <div id={generateId('main', 'container', 1)}>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
        {/* Executive Tabs Navigation */}
        <div id={generateId('navigation', 'tabs-container', 1)} className="tabs-executive flex justify-center mb-8">
          {tabs.map((tab, index) => {
            const Icon = tab.icon;
            return (
              <button
                key={tab.id}
                id={generateId('navigation', 'tab-button', index + 1)}
                onClick={() => setActiveTab(tab.id)}
                className={`tab-executive ${activeTab === tab.id ? 'active' : ''}`}
                data-testid={generateTestId('tab', tab.id)}
              >
      ...
      <TabsContent value="overview" className="animate-elegant">
        <div id={generateId('overview', 'grid', 1)} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Property Location */}
          <div id={generateId('overview', 'location-card', 1)} className="card-executive animate-elegant">
            <div id={generateId('location', 'header', 1)} className="elegant-card-header">
              <h3 className="elegant-card-title gold-accent">Property Location</h3>
            </div>
            <div id={generateId('location', 'content', 1)} className="pt-4">
              <div id={generateId('location', 'details', 1)} className="space-y-2">
                <div id={generateId('location', 'address', 1)}>
                ...
                <div id={generateId('location', 'info-grid', 1)} className="grid grid-cols-2 gap-2 pt-2 border-t border-gold">
                  <div id={generateId('location', 'parcel-info', 1)}>
                  ...
                  <div id={generateId('location', 'type-info', 1)}>
          ...
          {/* Property Values */}
          <div id={generateId('overview', 'values-card', 1)} className="card-executive animate-elegant">
            <div id={generateId('values', 'header', 1)} className="elegant-card-header">
              <h3 className="elegant-card-title gold-accent flex items-center">
                <DollarSign className="w-5 h-5 mr-2 text-gold" />
                Valuation Summary
              </h3>
            </div>
            <div id={generateId('values', 'content', 1)} className="pt-4">
              <div id={generateId('values', 'details', 1)} className="space-y-3">
                <div id={generateId('values', 'main-grid', 1)} className="grid grid-cols-2 gap-4">
                  <div id={generateId('values', 'just-value', 1)}>
                  ...
                  <div id={generateId('values', 'taxable-value', 1)}>
                ...
                <div id={generateId('values', 'breakdown-grid', 1)} className="grid grid-cols-2 gap-3 pt-3 border-t border-gold">
                  <div id={generateId('values', 'land-value-card', 1)} className="p-2 rounded-lg bg-gold-light">
                  ...
                  <div id={generateId('values', 'building-value-card', 1)} className="p-2 rounded-lg bg-gray-light">
      ...
      {['owner', 'sales', 'building', 'land', 'exemptions', 'notes'].map((tabId, index) => (
        <TabsContent key={tabId} value={tabId} className="animate-elegant">
          <div id={generateId('placeholder', 'card', index + 1)} className="card-executive animate-elegant text-center py-12">
            <div id={generateId('placeholder', 'header', index + 1)} className="elegant-card-header">
              <h3 className="elegant-card-title gold-accent capitalize">{tabId} Information</h3>
            </div>
            <div id={generateId('placeholder', 'content', index + 1)} className="pt-4">
