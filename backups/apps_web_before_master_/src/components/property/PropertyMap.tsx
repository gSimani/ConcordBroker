...
export function PropertyMap({ properties, onPropertySelect, onClose, selectedProperty, showingSelectedOnly = false, totalSelected = 0 }: PropertyMapProps) {
  const { generateId, generateTestId } = useElementId('property-map');
  ...
  if (error) {
    return (
      <div id={generateId('error', 'container', 1)} className="elegant-card">
        <CardHeader className="elegant-card-header">
          <div id={generateId('error', 'header', 1)} className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              onClick={onClose}
              data-testid={generateTestId('close', 'error-modal')}
            >
              <X className="w-4 h-4" />
            </Button>
          </div>
        ...
        <CardContent className="p-8 text-center">
          <div id={generateId('error', 'message', 1)} className="text-red-500 mb-4">
  ...
  return (
    <div id={generateId('main', 'container', 1)} className="elegant-card hover-lift animate-in">
      <div id={generateId('main', 'header', 1)} className="elegant-card-header">
        <div id={generateId('header', 'title-bar', 1)} className="flex items-center justify-between">
          <div id={generateId('header', 'title-section', 1)} className="flex items-center">
            <MapPin className="w-5 h-5 mr-2" style={{color: '#2c3e50'}} />
          ...
          <Button
            ...
            data-testid={generateTestId('close', 'map-modal')}
          >
            <X className="w-4 h-4" style={{color: '#2c3e50'}} />
          </Button>
        </div>
        <p id={generateId('header', 'description', 1)} className="text-sm mt-3" style={{color: '#7f8c8d'}}>
          {showingSelectedOnly
      ...
      <div id={generateId('main', 'content', 1)} className="relative">
        {isLoading && (
          <div id={generateId('content', 'loading-overlay', 1)} className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
            <div id={generateId('loading', 'spinner-container', 1)} className="text-center">
        ...
        <div
          id={generateId('content', 'map-canvas', 1)}
          ref={mapRef}
        ...
        >
          ...
          {properties.length > 0 ? (
            <>
              ...
              <div id={generateId('overlay', 'property-list', 1)} className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-4 max-w-sm max-h-96 overflow-y-auto">
                <h4 id={generateId('overlay', 'list-title', 1)} className="font-semibold text-sm mb-3" style={{color: '#2c3e50'}}>
                  Selected Properties ({properties.length})
                </h4>
                <div id={generateId('overlay', 'properties-container', 1)} className="space-y-2">
                  {properties.map((property, index) => (
                    <div
                      key={property.id}
                      id={generateId('overlay', 'property-item', index + 1)}
                      ...
                      data-testid={generateTestId('property', `select-${index + 1}`)}
                    >
                      <div id={generateId('property', 'item-content', index + 1)} className="flex items-start space-x-2">
                        <span className="text-xs font-bold" style={{color: '#d4af37'}}>
                          {index + 1}
                        </span>
                        <div id={generateId('property', 'details', index + 1)} className="flex-1">
                ...
                <div id={generateId('overlay', 'actions', 1)} className="mt-3 pt-3 border-t">
                  <button
                    onClick={() => {
                    ...
                    data-testid={generateTestId('open', 'route-google-maps')}
                  >
            ...
            <div id={generateId('content', 'empty-state', 1)} className="flex items-center justify-center h-full">
              <div id={generateId('empty', 'message', 1)} className="text-center">
        ...
        {/* Map Controls */}
        <div id={generateId('controls', 'container', 1)} className="absolute top-4 right-4 flex flex-col space-y-2">
          <Button
            size="sm"
            ...
            data-testid={generateTestId('zoom', 'in')}
          >
          ...
          <Button
            ...
            style={{color: '#2c3e50', border: '1px solid #ecf0f1'}}
            onClick={() => mapInstanceRef.current?.setZoom(mapInstanceRef.current.getZoom() - 1)}
            data-testid={generateTestId('zoom', 'out')}
          >
          ...
          <Button
            ...
            data-testid={generateTestId('locate', 'user-position')}
          >
