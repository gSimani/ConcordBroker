"""
Robust Property Appraiser Upload Script
Uploads remaining counties (skips Broward which is already done)
With proper error handling, resume capability, and progress tracking
"""
...
def upload_county_data(county_folder: str, progress_file: str):
    """Upload data for a single county with better error handling"""
                ...
                for _, row in chunk.iterrows():
                    try:
                        ...
                        record = {
                            ...
                            'owner_state': safe_str(row.get('OWN_STATE'), 50),
                            ...
                            'phy_state': safe_str(row.get('PHY_STATE'), 50),
                            ...
                            # Sales - handle year only
                            'sale_price': safe_int(row.get('SALE_PRC1')),
                            'sale_year': safe_int(row.get('SALE_YR1')),  # Add sale year
                            # Skip sale_date for now since it needs full timestamp
                    ...
                    try:
                        ...
                        if len(records) > 1000:
                            ...
                            for i in range(0, len(records), 1000):
                                small_batch = records[i:i+1000]
                                try:
                                    result = client.table('florida_parcels').upsert(
                                        small_batch,
                                        on_conflict='parcel_id,county,year'
                                    ).execute()
                                    total_uploaded += len(small_batch)
                                except Exception as e2:
                                    errors.append(f"Small batch error: {str(e2)[:100]}")
...
def main():
    """Main execution"""
    print("=" * 60)
    print("FLORIDA PROPERTY APPRAISER DATA UPLOAD (RESUMED)")
    print("=" * 60)
    ...
    if all_errors:
        ...
        print("See upload_progress.txt for details")
