#!/usr/bin/env python3
"""
Live Property Data Loader for ConcordBroker - BROWARD COUNTY
Loads real NAL data to populate the properties page immediately
"""
...
def load_properties():
    print("Loading Broward County Properties...")
    print("=====================================")
    ...
    try:
        ...
        # Read NAL data (limit to 5000 records for fast loading)
        print(f"Reading NAL file: {NAL_FILE}")
        df = pd.read_csv(NAL_FILE, dtype=str, nrows=5000)
        print(f"Loaded {len(df)} records")

        # Show available columns
        print("Available columns:", list(df.columns))

        # Process records with proper column mapping
        records = []
        for i, row in df.iterrows():
            try:
                ...
                record = {
                    ...
                    'property_city': clean_text(row.get('PHY_CITY')),
                    'property_zip': clean_text(row.get('PHY_ZIPCD')),
        ...
        print("Your database now has live property data!")
        print("\nNext steps:")
