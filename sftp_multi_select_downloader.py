"""
SFTP Multi-Select Downloader
Downloads 4 files at a time using Ctrl+Click multi-select
Handles session timeout popups
"""
...
class MultiSelectSFTPDownloader:
    def __init__(self):
    ...
    def log(self, message, level="INFO"):
        ...
        prefix = {"SUCCESS": "✅", "ERROR": "❌", "DOWNLOAD": "📥", "INFO": "ℹ️"}.get(level, "")
    ...
    async def handle_timeout_popup(self):
        ...
        try:
            ...
            continue_button = await self.page.query_selector('button:has-text("Continue")', timeout=1000)
            if continue_button:
    ...
    async def initialize_browser(self):
        ...
        self.log("Browser is VISIBLE - Watch the multi-select downloads!", "SUCCESS")
    ...
    async def download_with_multi_select(self, folder_path):
        """Download files using multi-select (Ctrl+Click)"""
        self.log(f"\n{'='*60}", "INFO")
        self.log(f"MULTI-SELECT DOWNLOAD FROM: {folder_path}", "INFO")
        self.log(f"{'='*60}", "INFO")
        ...
        for i in range(0, len(file_rows), self.batch_size):
            ...
            self.log(f"\n📦 Batch {batch_num}/{total_batches} - Selecting {len(batch)} files with Ctrl+Click:", "INFO")
            ...
            try:
                ...
                # All files are now selected (highlighted in blue)
                self.log(f"   ✓ {len(files_to_download)} files selected", "SUCCESS")
                
                # Now click the Download button ONCE for all selected files
                self.log("   📥 Looking for Download button...", "DOWNLOAD")
                ...
                if not download_button:
                    # Method 2: Look for button with download icon class
                    download_button = await self.page.query_selector('button.btn-primary:has-text("Download")')
                    
                if not download_button:
                    # Method 3: Look by button position (6th button in toolbar)
                ...
                if download_button:
                    self.log("   ✓ Found Download button, clicking now...", "SUCCESS")
                    
                    # Make sure button is visible and enabled
                    await download_button.wait_for_element_state('visible')
                    await download_button.wait_for_element_state('enabled')
                    
                    # Click with force if needed
                    await download_button.click(force=True)
                    self.log("   ✓ Download button CLICKED!", "SUCCESS")
                    
                    # Wait a moment for downloads to start
                    await asyncio.sleep(2)
                    
                    # Now handle the downloads
                    # When multi-selecting, browser might download files individually or as a zip
                    # Let's monitor the Downloads folder and move files to correct location
                    ...
                    await asyncio.sleep(5)  # Give time for downloads
                    
                    # Look for the downloaded files in Downloads folder
                    for filename in batch_filenames:
                        ...
                        if download_file.exists():
                            ...
                            self.log(f"      ⚠️ File not found: {filename}", "INFO")
    ...
    async def run(self):
        """Main execution"""
        print("="*60)
        print("MULTI-SELECT SFTP DOWNLOADER")
        print("="*60)
        print("📺 Browser stays VISIBLE")
        print("🖱️ Uses Ctrl+Click to select 4 files at once")
        print("📥 Downloads batch with single click")
        ...
        try:
            ...
            for folder in folders_to_download:
                await self.download_with_multi_select(folder)
...
async def main():
    downloader = MultiSelectSFTPDownloader()
    await downloader.run()

if __name__ == "__main__":
    print("\n🚀 Starting Multi-Select SFTP Downloader")
    print("This will:")
    print("  🖱️ Use Ctrl+Click to select 4 files at once")
    print("  📥 Download all selected with ONE click")
    print("  ⏰ Handle session timeout popups")
    print("  📁 Save to correct folders")
    print("-" * 60)
