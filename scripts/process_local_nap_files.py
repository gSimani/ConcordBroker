"""
Process all local NAP (Name, Address, Property characteristics) files and update Supabase florida_parcels table
This script processes existing NAP CSV files and enhances property data with additional characteristics
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('process_local_nap.log'),
...
# Path to NAP data
NAP_BASE_PATH = Path(r"C:\Users\gsima\Documents\MyProject\ConcordBroker\TEMP\DATABASE PROPERTY APP")
...
def process_nap_record(row: Dict, county_name: str, county_no: str) -> Optional[Dict]:
    """Process a single NAP record into database format - only existing columns"""
    try:
        # Get parcel ID (ACCT_ID in NAP corresponds to parcel_id)
        parcel_id = safe_string(row.get('ACCT_ID', '')).strip()
        if not parcel_id:
            return None
        ...
        nap_data = {
            ...
            # Enhanced value fields from NAP (existing columns)
            'just_value': safe_int(row.get('JV_TOTAL', 0)),
            'assessed_value': safe_int(row.get('AV_TOTAL', 0)),
            'taxable_value': safe_int(row.get('TAX_VAL', 0)),

            # Enhanced owner information from NAP (existing columns)
            'owner_name': safe_string(row.get('OWN_NAM', ''), 255),
            'owner_addr1': safe_string(row.get('OWN_ADDR', ''), 255),
        ...
        logger.error(f"Error processing NAP record: {str(e)}")
        return None

def update_parcels_with_nap(nap_data: List[Dict], batch_size: int = 500):
    """Update florida_parcels records with NAP data using upsert"""
    if not nap_data:
        return 0, 0
    ...
    try:
        # Process in batches
        for i in range(0, len(nap_data), batch_size):
            batch = nap_data[i:i + batch_size]

            # Update records using upsert (insert or update)
            try:
                # For existing records, we want to update them with NAP data
                # Use upsert with parcel_id, county, year as the conflict resolution
        ...
        return 0, len(nap_data)

def process_county_nap(county_path: Path) -> Dict:
    """Process all NAP files for a county"""
    ...
    # Find NAP CSV files
    nap_files = list(county_path.glob("NAP/*.csv"))
    if not nap_files:
        nap_files = list(county_path.glob("NAP/*.txt"))

    if not nap_files:
        logger.warning(f"No NAP files found for {county_name}")
        return stats

    for nap_file in nap_files:
        logger.info(f"Processing {county_name}: {nap_file.name}")
        ...
        try:
            ...
            for encoding in encodings:
                try:
                    with open(nap_file, 'r', encoding=encoding) as f:
                        # Detect delimiter
                        ...
                        for row_num, row in enumerate(reader):
                            ...
                            # Process record
                            nap_data = process_nap_record(row, county_name, county_no)
                            if nap_data:
                                nap_batch.append(nap_data)

                            stats['records_processed'] += 1

                            # Update in batches
                            if len(nap_batch) >= 1000:
                                updated, errors = update_parcels_with_nap(nap_batch)
            ...
            logger.error(f"Error processing {nap_file}: {str(e)}")
            stats['errors'] += 1
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PROCESSING LOCAL FLORIDA NAP FILES TO ENHANCE PARCEL DATA")
    logger.info("=" * 80)
    ...
    county_dirs = [d for d in NAP_BASE_PATH.iterdir() if d.is_dir()]
    ...
    for county_path in tqdm(county_dirs, desc="Processing counties"):
        ...
        stats = process_county_nap(county_path)
    ...
    # Log summary
    logger.info("=" * 80)
    logger.info("NAP PROCESSING COMPLETE")
    logger.info("=" * 80)
    ...
    # Save summary
    summary_file = Path("local_nap_processing_summary.txt")
    with open(summary_file, 'w') as f:
        f.write(f"Florida NAP Processing Summary\n")
