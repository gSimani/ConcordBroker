"""
Process all local SDF files and import to Supabase property_sales_history table
This script processes existing SDF CSV files without Excel row limitations
"""
...
# Using Supabase client instead of direct PostgreSQL
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('process_local_sdf.log'),
...
def process_sdf_record(row: Dict, county_name: str, county_no: str) -> List[Dict]:
    ...
    if 'SALE_YR' in row and 'SALE_PRC' in row:
        sale_price = safe_int(row.get('SALE_PRC', 0))
        if sale_price >= 1000:
            ...
            if sale_date:
                sales.append({
                    'parcel_id': parcel_id,
                    'county_no': co_no,
                    'sale_year': safe_int(row.get('SALE_YR', 0)),
                    'sale_month': safe_int(row.get('SALE_MO', 1)),
    ...
    for i in range(1, 4):  # Check up to 3 sales
        ...
        sale_price = safe_int(row.get(price_field, 0))
        if sale_price < 1000:
            continue
        ...
        if sale_date:
            sales.append({
                'parcel_id': parcel_id,
                'county_no': co_no,
                'sale_year': safe_int(row.get(year_field, 0)),
                'sale_month': safe_int(row.get(f'SALE_MO{i}', 1)),
                'sale_price': sale_price,
...
def process_county_sdf(county_path: Path) -> Dict:
    ...
    # Find SDF CSV files
    sdf_files = list(county_path.glob("SDF/*.csv"))
    if not sdf_files:
        sdf_files = list(county_path.glob("SDF/*.txt"))
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PROCESSING LOCAL FLORIDA SDF FILES")
    logger.info("=" * 80)
    ...
    # Connection handled by Supabase client
    ...
    # Save summary
    summary_file = Path("local_sdf_processing_summary.txt")
    with open(summary_file, 'w') as f:
        f.write(f"Florida SDF Processing Summary\n")
