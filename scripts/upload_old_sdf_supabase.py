#!/usr/bin/env python3
"""
Upload old SDF (sales) data into Supabase safely.

Features
- Parses SDF and uploads to public.property_sales_history using ON CONFLICT
  (strong key by default): (parcel_id, county, sale_date, sale_price, or_book, or_page, clerk_no).
...
Usage:
- python scripts/upload_old_sdf_supabase.py \
    --year 2023 \
    --folders "C:\\path\\to\\2023SDF" \
    --batch-size 500
"""
...
def main():
    parser = argparse.ArgumentParser(description="Upload old SDF (sales) files into Supabase")
    parser.add_argument('--year', type=int, required=True, help='Sales year for SDF (e.g., 2023)')
    ...
    parser.add_argument('--simple-key', action='store_true', help='Use simpler ON CONFLICT key (parcel_id,county,sale_date)')
    ...
    strong_key = not args.simple_key
    ...
    try:
        ...
        if args.progress and total_considered > 0:
            try:
                ing.cursor.execute(
                    ...
                    (county_filter or None, {"start": year, "end": year}, 0, total_considered, 0, f"start:SDF {year}")
                )
                ing.conn.commit()
            except Exception:
                pass
        ...
        for folder, csv, c in considered:
            try:
                    df = ing.parse_sdf_file(csv, c, year)
                    if df is None or len(df) == 0:
                        continue
                    if args.limit:
                        df = df.head(args.limit)
                    uploaded = ing.upload_sdf_to_postgres(df, batch_size=batch_size, use_strong_key=strong_key)
                    total_rows += uploaded
                    total_files += 1
                    done_files += 1
                    if args.progress and total_considered > 0:
        ...
        print(f"\n=== SDF upload complete: files={total_files}, rows={total_rows:,} ===")
    finally:
        ing.disconnect()
