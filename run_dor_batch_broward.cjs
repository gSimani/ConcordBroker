/**
 * DOR Code Batch Assignment Runner - BROWARD County
...
async function getCountyCoverage() {
  try {
    // Get total
...
async function main() {
  ...
  console.log('DOR CODE BATCH ASSIGNMENT RUNNER - BROWARD COUNTY');
  console.log('='.repeat(80));
  ...
  // Initial stats
  const initialStats = await getCountyCoverage();
  ...
  while (callCount < MAX_CALLS) {
    ...
    if (callCount % CALLS_PER_UPDATE === 0) {
      const currentStats = await getCountyCoverage();
      console.log('\n' + '-'.repeat(80));
  ...
  // Final stats
  console.log('\n' + '='.repeat(80));
  console.log('BATCH ASSIGNMENT COMPLETE - BROWARD COUNTY');
  console.log('='.repeat(80));

  const finalStats = await getCountyCoverage();

  console.log(`\nTotal calls made: ${callCount}`);
  console.log(`Total properties processed: ${totalProcessed.toLocaleString()}`);
  console.log(`\nFinal BROWARD Coverage:`);
