    ...
    try {
        ...
        for (let i = 0; i < tabs.length; i++) {
            ...
            if (tabText === 'Core Property Info') {
                ...
                const naValues = await page.$$eval('*', elements => {
                    return elements
                        .filter(el => el.textContent && el.textContent.includes('N/A'))
                        .map(el => ({
                ...
                if (mostRecentSale) {
                    ...
                    const saleData = await page.evaluate(() => {
                        ...
                        for (let el of elements) {
                            if (el.textContent && el.textContent.includes('Most Recent Sale')) {
                                const section = el.closest('.sale-section, .recent-sale, [data-testid*="recent-sale"]') || el.parentElement;
                                return section ? section.textContent : 'Section found but no data extracted';
                            }
                        }
                        return 'Most Recent Sale text found but no parent section';
                    });
        ...
        const dollarAmounts = await page.$$eval('*', elements => {
            return elements
        ...
        // Look for dates
        const dates = await page.$$eval('*', elements => {
            return elements
                .filter(el => el.textContent && /\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}/.test(el.textContent))
                .map(el => el.textContent.match(/\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}/g))
                .flat()
                .filter(Boolean);
        });
        
        // Look for all N/A values
        const allNAValues = await page.$$eval('*', elements => {
            return elements
                .filter(el => el.textContent && el.textContent.includes('N/A'))
                .map(el => ({
                    text: el.textContent.trim(),
