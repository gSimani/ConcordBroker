"""
Florida Revenue NAL (Name Address Library) Counties Downloader
Downloads NAL files for all Florida counties from the state data portal
"""
...
class FloridaNALCountiesDownloader:
    """Downloads NAL data files for all Florida counties"""
    ...
    def __init__(self, data_dir: str = "data/florida_nal_counties"):
        """
        Initialize the NAL counties downloader
    ...
    def get_nal_file_url(self, county_code: str, year: Optional[int] = None) -> str:
        """
        Construct NAL file URL for a specific county
        ...
        Returns:
            URL for the NAL file
        """
        if not year:
            year = datetime.now().year
        
        # NAL file naming pattern: NAL[COUNTY_CODE]P[YEAR].txt
        # Example: NAL06P2025.txt for Broward County 2025
        filename = f"NAL{county_code}P{year}.txt"
        
        # Construct full URL
        url = f"{self.BASE_URL}NAL/{year}P/{filename}"
    ...
    def download_county_nal(self, county_code: str, year: Optional[int] = None, force: bool = False) -> Optional[Dict]:
        """
        ...
        # Get file URL
        url = self.get_nal_file_url(county_code, year)
        
        logger.info(f"Checking NAL file for {county_name} ({county_code}) - Year {year}")
        
        # Check if file exists
        exists, file_info = self.check_file_exists(url)
        
        if not exists:
            logger.warning(f"NAL file not found for {county_name} at {url}")
            return None
        ...
        if not force and download_key in self.download_history['downloads']:
            ...
            if (previous.get('size') == file_info['size'] and
                ...
                logger.info(f"NAL file for {county_name} is up to date")
                return {
                    'status': 'up_to_date',
        ...
        # Download the file
        logger.info(f"Downloading NAL file for {county_name} ({file_info['size']:,} bytes)")
        ...
        try:
            ...
            logger.info(f"Successfully downloaded NAL file for {county_name}")
            ...
            logger.error(f"Failed to download NAL file for {county_name}: {e}")
    ...
    def download_all_counties(self, year: Optional[int] = None, counties: Optional[List[str]] = None) -> Dict:
        """
        Download NAL files for all or specified counties
        ...
        logger.info(f"Starting download of NAL files for {len(counties)} counties - Year {year}")
        ...
        for i, county_code in enumerate(sorted_counties, 1):
            ...
            result = self.download_county_nal(county_code, year)
    ...
    def download_priority_counties(self, year: Optional[int] = None) -> Dict:
        """Download NAL files for priority counties only"""
    ...
    def check_for_updates(self, year: Optional[int] = None) -> List[str]:
        """
        Check which counties have updated NAL files
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Florida NAL Counties Downloader')
    ...
    downloader = FloridaNALCountiesDownloader()
    ...
    if args.status:
        ...
        result = downloader.download_county_nal('06', args.year)
        if result:
