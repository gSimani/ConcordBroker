"""
Florida Revenue SDF (Sales Data File) Counties Downloader
Downloads SDF files for all 67 Florida counties from the state data portal
"""
...
class FloridaSDFCountiesDownloader:
    """Downloads SDF sales data files for all Florida counties"""
    ...
    # Florida counties with codes (01-67)
    FLORIDA_COUNTIES = {
    ...
    # Priority counties (major population centers with most property sales)
    PRIORITY_COUNTIES = ['06', '13', '29', '48', '50', '52']  # Broward, Miami-Dade, Hillsborough, Orange, Palm Beach, Pinellas
    
    def __init__(self, data_dir: str = "data/florida_sdf_counties"):
        """
    ...
    def get_sdf_file_url(self, county_code: str, year: Optional[int] = None) -> str:
        """
        Construct SDF file URL for a specific county
        
        Args:
            county_code: Two-digit county code (01-67)
        ...
        # SDF file naming pattern: SDF[CC]P[YYYYMM].txt
        # Located in: SDF/[YEAR]P/SDF[CC]P[YEAR]01.txt
        # Note: P suffix indicates production data, 01 indicates January
        filename = f"SDF{county_code}P{year}01.txt"
        url = f"{self.BASE_URL}SDF/{year}P/{filename}"
    ...
    def download_county_sdf(self, county_code: str, year: Optional[int] = None, force: bool = False) -> Optional[Dict]:
        """
        ...
        # Get file URL
        url = self.get_sdf_file_url(county_code, year)
        
        logger.info(f"Checking SDF file for {county_name} ({county_code}) - Year {year}")
        
        # Check if file exists
        exists, file_info = self.check_file_exists(url)
        
        if not exists:
            logger.warning(f"SDF file not found for {county_name} at {url}")
            return None
        ...
        if not force and download_key in self.download_history['downloads']:
            ...
            if (previous.get('size') == file_info['size'] and
                ...
                logger.info(f"SDF file for {county_name} is up to date")
                return {
                    'status': 'up_to_date',
        ...
        # Download the file
        logger.info(f"Downloading SDF file for {county_name} ({file_info['size']:,} bytes)")
        ...
        try:
            ...
            logger.info(f"Successfully downloaded SDF file for {county_name}")
            ...
            logger.error(f"Failed to download SDF file for {county_name}: {e}")
    ...
    def download_all_counties(self, year: Optional[int] = None, counties: Optional[List[str]] = None) -> Dict:
        """
        Download SDF files for all or specified counties
        ...
        logger.info(f"Starting download of SDF files for {len(counties)} counties - Year {year}")
        ...
        for i, county_code in enumerate(counties, 1):
            county_name = self.FLORIDA_COUNTIES.get(county_code, county_code)
            logger.info(f"Processing {i}/{len(counties)}: {county_name} ({county_code})")
            
            result = self.download_county_sdf(county_code, year)
    ...
    def download_priority_counties(self, year: Optional[int] = None) -> Dict:
        """Download SDF files for priority counties only"""
        logger.info("Downloading SDF files for priority counties")
    ...
    def check_for_updates(self, year: Optional[int] = None) -> List[str]:
        """
        Check which counties have updated SDF files
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Florida SDF Counties Downloader')
    parser.add_argument('--counties', nargs='+', help='County codes to download (01-67)')
    ...
    downloader = FloridaSDFCountiesDownloader()
    ...
    if args.status:
        ...
        if status['counties_missing']:
            print(f"  Missing counties: {', '.join(status['counties_missing'][:10])}")
            if len(status['counties_missing']) > 10:
                print(f"    ... and {len(status['counties_missing']) - 10} more")
        ...
        if updates:
            print(f"\nCounties with SDF updates: {len(updates)}")
            for code in updates[:10]:
                name = downloader.FLORIDA_COUNTIES.get(code, code)
                print(f"  - {name} ({code})")
            if len(updates) > 10:
                print(f"  ... and {len(updates) - 10} more")
        else:
            print("No SDF updates available")
        ...
        result = downloader.download_county_sdf('06', args.year)
        if result:
