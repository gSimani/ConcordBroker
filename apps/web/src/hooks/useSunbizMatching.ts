...
export function useSunbizMatching(parcelId: string, ownerName?: string, autoFetch: boolean = false) {
  ...
  const fetchSunbizMatches = async () => {
    if (!parcelId || !ownerName) {
      setError('Missing parcel ID or owner name');
      return;
    }
  ...
  // Only auto-fetch if explicitly enabled (for backwards compatibility)
  useEffect(() => {
    if (autoFetch && parcelId && ownerName) {
      fetchSunbizMatches();
    }
  }, [parcelId, ownerName, autoFetch]);
  ...
  return {
    ...
    fetchMatches: fetchSunbizMatches, // Export the manual fetch function
  };
}
