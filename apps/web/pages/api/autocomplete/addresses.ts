...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Try to connect to the backend API on Railway
    const backendUrl = process.env.BACKEND_API_URL || 'https://concordbroker-api.up.railway.app'

    try {
      const response = await fetch(`${backendUrl}/api/autocomplete?q=${encodeURIComponent(q)}&limit=${limit}`)
      if (response.ok) {
    ...
    // Fallback to mock AI-enhanced autocomplete data with business entities
